...
Stokes (2016) uses a natural experiment to demonstrate how  citizens hold incumbent governments accountable because of its climate policy, leading to electoral losses for the incumbent party ranging from 4 to 10%, with the effect persisting 3 km from wind turbines.
...
-   Plot the distributions of your variables, and a diagrams to show the relationship between the variables of interest. Assuming that the data was observed (rather than simulated), how would you know that  your instrument is *relevant* and *valid*? 
...
2. Using data visualization, investigate the distribution of some of the variables of interest, and the relationship between them. Does the data suggest that the instrument is relevant and valid? Discuss. 

3. Assess whether wind speed can be considered to be as-if randomly assigned geographically, by regressing avg pwr log on all of the geographical covariates. What do you conclude? Is that problematic in terms of relevance or validity of the instrument? 

*Code Hint:* Remember to use factor() for the ed id variable

4. Estimate the first-stage relationship between treatment and the instrument using a regression with no added covariates. Interpret the result precisely.
...
6. Estimate the first-stage relationship between the treatment and the instrument using a regression, this time with a full set of geographic controls. Interpret the result, and explain why it does or does not differ from part your answer to question 4. 
