...
Y &\sim \mathcal{N}(\mu=\beta_1\cdot X+\beta_2\cdot U, \sigma=1)  \\
\end{aligned}
$$
...
-   Without using the unobserved confounder, use the instrumentto produce a model with unbiased estimates of the effect of treatment on the outcome. Estimate the effect using the 2SLS approach by hand, and by using the `ivreg`  function from the **AER** library. Note that `ivreg` formula syntax is `Y ~ T  | Z `, where `T` is treatment, and `Z` is the instrument.
