...
f.posterior <- function(obs, prior) {
  ...
  # Now multiply the likelihood by the prior, and 
...
d <- data.frame(hyp.range, post1, 
                post2, post3, 
                post4, post5) %>% 
  pivot_longer(cols = c("post1", "post2", 
                        "post3", "post4", 
                        "post5")) %>% mutate(posterior = factor(name, levels = c("post1", "post2", "post3", "post4", "post5")))

ggplot(d) + 
  geom_line(aes(x=hyp.range, y=value, color=posterior)) + 
  scale_x_continuous(breaks=seq(50,350,by=50), limits=c(50,360), name="hypothesized number of tanks") + 
...
Please see below the code needed to plot this graph. 

```{r bonus-show, echo=TRUE, eval=FALSE}

# Fill this in with the number of observations, 
# Or how many tanks you captured
n.observations <- ____
...
percentile.90 <- data.frame(min.90=rep(NA,____), 
                            max.90=rep(NA,____))
# We do need our prior!
prior <- rep(1, max.n)
set.seed(____)
for(i in 1:____){
  # observe a random tank from a uniform discrete distribution
  obs <- sample(1:n.tanks, 1)
  
  # find the posterior for our observation
  post <- f.posterior(obs, prior)
  
  # find the first hypothesis whose posterior is greater than zero
  first.hyp.nonzero.posterior <- min(which(post>____))
  
  # find the last hypothesis whose cumulative 
  # posterior is less than 90%
  last.hyp.cum.post.in.90pct <- max(which(cumsum(post)<____))
  
  # fill in the 90th percentile for the current observation
  percentile.90[i,] <- 
    c(________,       # the first hypothesis in the 90th percentile
      ________)       # the last hypothesis in the 90th percentile
  
  # Use the current posterior as the prior for the next observation
  prior <- ________
}

# now let's plot it!
ggplot() + 
  geom_ribbon(aes(x=1:________, 
                   ymin=percentile.90$________, 
                   ymax=percentile.90$________), alpha=.4) + 
  geom_hline(yintercept = n.tanks, color="red",  alpha=.6) 
...
```{r bonus-answer, echo=FALSE, eval=FALSE}

# Fill this in with the number of observations, 
# Or how many tanks you captured
n.observations <- 50

# The data frame below is a place holder for three columns: 
# 1. the observation number running from 1 to n.observations
# 2. The minimum value of our 90% interval
# 3. The maximum value of our 90% interval
percentile.90 <- data.frame(min.90=rep(NA,n.observations), 
...
for(i in 1:n.observations){
  ...
  # find the first hypothesis whose posterior is greater than zero
  first.hyp.nonzero.posterior <- min(which(post>0))
  
  # find the first hypothesis whose cumulative 
  # posterior is greater than 90%
  first.hyp.cum.post.in.90pct <- min(which(cumsum(post)>0.9))
  
  # fill in the 90th percentile for the current observation
  percentile.90[i,] <- 
    c(first.hyp.nonzero.posterior, 
      first.hyp.cum.post.in.90pct)
...
ggplot() + 
  # geom_point(aes(x=index, y=obs), color="red", size=1) +
  geom_ribbon(aes(x=1:n.observations, 
                   ymin=percentile.90$min.90, 
                   ymax=percentile.90$max.90), alpha=.4) + 
  geom_hline(yintercept = n.tanks, color="red", alpha=.6) + 
  scale_y_log10(name="90% credibility interval", 
                breaks=c(200,246, 300, 400, 500, 800)) + 
  scale_x_log10(name="observation number", 
